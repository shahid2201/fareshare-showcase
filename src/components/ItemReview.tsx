...
import { CheckCircle2, Pencil, Sparkles, Users } from "lucide-react";

import { SITE_DISCLAIMERS } from "@/lib/disclaimers";
import { MarketingDisclaimer } from "./MarketingDisclaimer";

const items = [
  { name: "Organic Milk", price: 4.99, status: "Ready", category: "Dairy", color: "bg-blue-500" },
  { name: "Sourdough Bread", price: 5.49, status: "Ready", category: "Bakery", color: "bg-amber-500" },
  { name: "Avocados (3)", price: 3.99, status: "Review", category: "Produce", color: "bg-green-500" },
  { name: "Trail Mix", price: 6.99, status: "Ready", category: "Snacks", color: "bg-orange-500" },
  { name: "Granola Bars", price: 4.49, status: "Unassigned", category: "Snacks", color: "bg-orange-500" },
  { name: "Sparkling Water", price: 3.29, status: "Ready", category: "Beverages", color: "bg-cyan-500" },
];

const reviewActions = [
  { icon: Pencil, label: "Edit items" },
  { icon: Users, label: "Batch assign" },
  { icon: CheckCircle2, label: "Confirm scan" },
];

function statusTone(status: string) {
  if (status === "Ready") return "bg-emerald-500/20 text-emerald-400";
  if (status === "Review") return "bg-amber-500/20 text-amber-400";
    ...
    <section id="review" className="section-padding relative overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        >
          ...
          >
            Smart reads.{" "}
            <span className="gradient-text">Your final say.</span>
          </h2>
          <p className="mx-auto mt-4 max-w-2xl text-base text-zinc-400">
            Every scan lands in a review screen where you fix names, check prices, and assign
            shares — nothing goes to friends until you confirm.
          </p>
          <MarketingDisclaimer className="mx-auto mt-4 max-w-2xl text-center">
            {SITE_DISCLAIMERS.ocr}
          </MarketingDisclaimer>
        </motion.div>
          ...
          >
            ...
            <div className="mb-6 flex gap-3">
              {reviewActions.map((action) => (
                <motion.div
                  ...
                  className="flex flex-1 flex-col items-center gap-2 rounded-xl border border-white/5 bg-white/[0.03] p-4"
                >
            ...
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
              ...
              <p className="text-sm text-zinc-300">
                2 items still unassigned on this receipt — tap to assign them now.
              </p>
              <div className="mt-3 flex gap-2">
                <button type="button" className="rounded-lg bg-emerald-500/20 px-3 py-1.5 text-xs font-medium text-emerald-400">
                  Assign items
                </button>
                <button type="button" className="rounded-lg px-3 py-1.5 text-xs text-zinc-500">
                  Later
                </button>
              </div>
            </div>
