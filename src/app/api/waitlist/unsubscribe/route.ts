...
export async function GET(request: Request) {
  ...
  return NextResponse.redirect(
    new URL(
      `/waitlist/unsubscribe?token=${encodeURIComponent(token)}`,
      request.url,
    ),
  );
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await unsubscribeWaitlistByToken(supabase, token);
    ...
    if (result === "already_unsubscribed") {
      return jsonSuccess({ ok: true, state: "already_unsubscribed" });
    }

    return jsonSuccess({ ok: true, state: "unsubscribed" });
  } catch (error) {
